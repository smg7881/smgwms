...
## 1. 문서 개요
- 화면명: 매출요율관리
- 화면 ID: `sellFeeRtMngt.dev`
- 기준 문서:
  - `LogisT-WM-AN13(화면정의서-V1.0)-매출요율관리.pdf`
  - `LogisT-WM-DS02(화면설계서-V1.0)-매출요율관리.pdf`
- 적용 범위: WMS > 매출요율관리 (`/wm/sell_fee_rt_mngs`)
- 개발 패턴: `master-detail-screen-pattern` 기반 (Master 1:N Detail)

## 2. 핵심 요구사항
1. 매출요율 마스터/상세를 1:N 구조로 관리한다.
2. 저장 버튼은 마스터 영역에 1개만 제공한다.
3. 저장 시 마스터/디테일 변경분을 한 번의 요청으로 동시 저장한다.
4. 공통코드는 숫자 코드를 사용한다.
  - 사용여부/자동여부/확정여부: `06`
  - 매출단위분류: `20`
  - 통화코드: `27`
...
1. 화면 진입
  - 조회조건 기본값:
    ...
    - 적용일자 To = 오늘
2. 조회조건 입력 후 검색
  - 조건: 작업장, 계약고객, 매출항목, 사용여부, 적용일자(From~To)
  - 결과: 마스터 그리드(요율목록) 표시
3. 마스터 행 선택
...
3. 마스터 수정 가능 항목:
  - 매출아이템유형, 매출아이템, 매출단위분류, 매출단위, 사용여부, 자동여부, 비고
4. 디테일 수정 가능 항목:
  - 전체 컬럼 수정 가능

## 6. 데이터 모델
### 6.1 마스터 테이블
- 테이블명: `wm_sell_fee_rt_mngs`
- 키: `wrhs_exca_fee_rt_no` (PK)
- 주요 컬럼:
  - `corp_cd`, `work_pl_cd`, `sell_buy_sctn_cd(고정:10)`, `ctrt_cprtco_cd`, `sell_buy_attr_cd`
  - `sell_dept_cd`, `sell_item_type`, `sell_item_cd`, `sell_unit_clas_cd`, `sell_unit_cd`
  - `use_yn`, `auto_yn`, `rmk`
  - `create_by`, `create_time`, `update_by`, `update_time`

### 6.2 상세 테이블
- 테이블명: `wm_sell_fee_rt_mng_dtls`
...
## 7. API 요건
### 7.1 목록 조회
- `GET /wm/sell_fee_rt_mngs`
- `GET /wm/sell_fee_rt_mngs/:id/details`

### 7.2 통합 저장
- `POST /wm/sell_fee_rt_mngs/batch_save`
- 요청 본문:
...
6. 그리드 lookup 팝업(작업장/고객/매출항목/부서/아이템) 값 반영 확인
