class AddSystemCodeMenu < ActiveRecord::Migration[8.1]
  ...
  def up
    ...
    code_menu = MigrationAdmMenu.find_or_initialize_by(menu_cd: "SYS_CODE")
    code_menu.assign_attributes(
      menu_nm: "공통코드관리",
      parent_cd: "SYSTEM",
      menu_url: "/system/code",
      menu_icon: "hash",
  ...
  def down
    ...
    MigrationAdmMenu.where(menu_cd: "SYS_CODE").delete_all
  end
  ...
  private
    ...
    def next_system_child_sort_order
      MigrationAdmMenu.where(parent_cd: "SYSTEM").where.not(menu_cd: "SYS_CODE").maximum(:sort_order).to_i + 1
    end
end
