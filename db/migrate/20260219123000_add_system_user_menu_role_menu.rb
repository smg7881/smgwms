class AddSystemUserMenuRoleMenu < ActiveRecord::Migration[8.1]
  ...
  def up
    ...
    ensure_user_menu_role_menu!
  end

  def down
    return unless table_exists?(:adm_menus)

    MigrationAdmMenu.where(menu_cd: "SYS_USER_MENU_ROLE").delete_all
  end
  ...
  private
    ...
    def ensure_user_menu_role_menu!
      menu = MigrationAdmMenu.find_or_initialize_by(menu_cd: "SYS_USER_MENU_ROLE")
      menu.assign_attributes(
        menu_nm: "사용자별메뉴권한",
        parent_cd: "SYSTEM",
        menu_url: "/system/userMenuRole",
    ...
    def next_system_child_sort_order
      MigrationAdmMenu.where(parent_cd: "SYSTEM")
                      .where.not(menu_cd: "SYS_USER_MENU_ROLE")
