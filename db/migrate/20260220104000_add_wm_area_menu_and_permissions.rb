class AddWmAreaMenuAndPermissions < ActiveRecord::Migration[8.1]
  ...
  def down
    ...
    if table_exists?(:adm_user_menu_permissions)
      MigrationAdmUserMenuPermission.where(menu_cd: "WM_AREA").delete_all
    end

    MigrationAdmMenu.where(menu_cd: "WM_AREA").delete_all
  end
  ...
  private
    ...
    def ensure_area_menu!
      menu = MigrationAdmMenu.find_or_initialize_by(menu_cd: "WM_AREA")
    ...
    def ensure_area_permissions!
      return unless table_exists?(:adm_user_menu_permissions)

      now = Time.current

      MigrationUser.find_each do |user|
        permission = MigrationAdmUserMenuPermission.find_or_initialize_by(user_id: user.id, menu_cd: "WM_AREA")
        permission.use_yn = "Y"
