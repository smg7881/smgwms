class FixStdSellContractMenuCode < ActiveRecord::Migration[8.1]
  ...
  CORRECT_MENU_CD = "SALES_SELL_CONT"

  def up
    return unless table_exists?(:adm_menus)

    menu = MigrationAdmMenu.find_by(menu_cd: CORRECT_MENU_CD)
    if menu.nil?
      return
    end
