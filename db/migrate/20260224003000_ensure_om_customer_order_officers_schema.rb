class EnsureOmCustomerOrderOfficersSchema < ActiveRecord::Migration[8.1]
  TABLE_NAME = :om_customer_order_officers
  OLD_UNIQUE_INDEX = "idx_om_cust_ord_ofcr_unique"
  NEW_UNIQUE_COLUMNS = %i[ord_chrg_dept_cd cust_cd exp_imp_dom_sctn_cd cust_ofcr_nm].freeze
  ...
  def down
    ...
    if index_exists?(TABLE_NAME, NEW_UNIQUE_COLUMNS, name: OLD_UNIQUE_INDEX)
      remove_index TABLE_NAME, name: OLD_UNIQUE_INDEX
    end

    if column_exists?(TABLE_NAME, :cust_cd) && column_exists?(TABLE_NAME, :cust_ofcr_nm)
      if !index_exists?(TABLE_NAME, [ :cust_cd, :cust_ofcr_nm ], name: OLD_UNIQUE_INDEX)
        add_index TABLE_NAME, [ :cust_cd, :cust_ofcr_nm ], unique: true, name: OLD_UNIQUE_INDEX
      end
    end
  end
  ...
  private
    def ensure_columns!
      ...
      add_column TABLE_NAME, :exp_imp_dom_sctn_cd, :string, limit: 30 unless column_exists?(TABLE_NAME, :exp_imp_dom_sctn_cd)
      add_column TABLE_NAME, :cust_ofcr_nm, :string, limit: 100, null: false, default: "" unless column_exists?(TABLE_NAME, :cust_ofcr_nm)
      add_column TABLE_NAME, :cust_ofcr_tel_no, :string, limit: 30 unless column_exists?(TABLE_NAME, :cust_ofcr_tel_no)
      add_column TABLE_NAME, :cust_ofcr_mbp_no, :string, limit: 30 unless column_exists?(TABLE_NAME, :cust_ofcr_mbp_no)
    ...
    def normalize_existing_rows!
      ...
      if column_exists?(TABLE_NAME, :cust_ofcr_nm)
        execute("UPDATE #{TABLE_NAME} SET cust_ofcr_nm = '' WHERE cust_ofcr_nm IS NULL")
      end
    ...
    def ensure_indexes!
      ...
      if !index_exists?(TABLE_NAME, NEW_UNIQUE_COLUMNS, name: OLD_UNIQUE_INDEX)
        add_index TABLE_NAME, NEW_UNIQUE_COLUMNS, unique: true, name: OLD_UNIQUE_INDEX
      end
    end
end
