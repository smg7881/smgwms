class Wm::PurFeeRtMngsController < Wm::BaseController
  def index
  ...
  private
    ...
    def records_scope
      ...
      if search_params[:aply_date_from].present? && search_params[:aply_date_to].present?
        ...
        detail_table = Wm::PurFeeRtMngDtl.table_name
    ...
    def process_master_inserts(rows, result, errors, inserted_master_key_by_temp_id)
      Array(rows).each do |attrs|
        ...
        record = Wm::PurFeeRtMng.new(master_insert_attrs(attrs))
    ...
    def process_master_updates(rows, result, errors)
      Array(rows).each do |attrs|
        ...
        record = Wm::PurFeeRtMng.find_by(wrhs_exca_fee_rt_no: master_key)
    ...
    def process_master_deletes(rows, result, errors, deleted_master_keys)
      Array(rows).each do |raw_key|
        ...
        if record.destroy
          ...
          errors.concat(record.errors.full_messages.presence || [ "매입요율 삭제에 실패했습니다: #{master_key}" ])
        end
      end
    end
    ...
    def resolve_detail_master_key(detail_operations:, inserted_master_key_by_temp_id:, deleted_master_keys:, errors:)
      ...
      if master_key.present?
        if deleted_master_keys.include?(master_key)
          errors << "삭제된 매입요율은 상세를 저장할 수 없습니다."
          return nil
        end

        if Wm::PurFeeRtMng.exists?(wrhs_exca_fee_rt_no: master_key)
          return master_key
        end
      end
      ...
      errors << "상세 저장 대상 매입요율을 찾을 수 없습니다."
      nil
    end
    ...
    def process_detail_operations(master_key:, detail_operations:, result:, errors:)
      ...
      if master_key.blank?
        errors << "상세 저장 대상 매입요율이 없습니다."
        return
      end

      master = Wm::PurFeeRtMng.find_by(wrhs_exca_fee_rt_no: master_key)
      if master.nil?
    ...
    def master_insert_attrs(attrs)
      attrs.permit(
        :work_pl_cd, :ctrt_cprtco_cd, :sell_buy_attr_cd, :pur_dept_cd,
        :pur_item_type, :pur_item_cd, :pur_unit_clas_cd, :pur_unit_cd,
        :use_yn, :auto_yn, :rmk
      )
    end
    ...
    def generate_fee_rt_no
      "P#{Time.current.strftime('%Y%m%d%H%M%S')}#{rand(100..999)}"
    end
