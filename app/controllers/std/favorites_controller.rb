    ...
    def batch_save
      ...
      ActiveRecord::Base.transaction do
        ...
        Array(operations[:rowsToUpdate]).each do |attrs|
          user_id_code = attrs[:user_id_code].to_s.strip.upcase.presence || current_user_id_code
        ...
        Array(operations[:rowsToDelete]).each do |delete_key|
          ...
          user_id_code = key_hash[:user_id_code].to_s.strip.upcase.presence || current_user_id_code
  ...
  private
    ...
    def batch_save_params
      params.permit(
        ...
        rowsToInsert: [ :user_id_code, :menu_cd, :menu_nm, :user_favor_menu_grp, :sort_order, :use_yn ],
        rowsToUpdate: [ :user_id_code, :menu_cd, :menu_nm, :user_favor_menu_grp, :sort_order, :use_yn ]
      )
    end
    ...
    def favorite_params_from_row(row)
      row.permit(
        :user_id_code, :menu_cd, :menu_nm, :user_favor_menu_grp, :sort_order, :use_yn
      ).to_h.symbolize_keys
    end
    ...
    def favorite_json(row)
      menu = AdmMenu.find_by(menu_cd: row.menu_cd)
      parent_menu = menu&.parent_cd ? AdmMenu.find_by(menu_cd: menu.parent_cd) : nil
      {
        ...
        sort_order: row.sort_seq,
        use_yn: row.use_yn,
        tab_id: menu&.tab_id,
        menu_url: menu&.menu_url,
        child_sort_order: menu&.sort_order || 999,
        parent_cd: parent_menu&.menu_cd,
        up_menu_nm: parent_menu&.menu_nm || "기타",
        parent_sort_order: parent_menu&.sort_order || 999,
        create_by: row.create_by,
        create_time: row.create_time,
        update_by: row.update_by,
        update_time: row.update_time
