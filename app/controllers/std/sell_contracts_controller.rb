class Std::SellContractsController < Std::BaseController
  def index
    ...
    respond_to do |format|
      format.html
      format.json do
        rows = sell_contract_scope.to_a
        render json: rows.map { |row| sell_contract_json(row) }
      end
    end
  end
  ...
  def batch_save
    ...
    ActiveRecord::Base.transaction do
      Array(operations[:rowsToInsert]).each do |attrs|
        if attrs[:sell_ctrt_nm].to_s.strip.blank?
          next
        end

        record = StdSellContract.new(contract_params_from_row(attrs))
        if record.save
          result[:inserted] += 1
        else
      ...
      Array(operations[:rowsToUpdate]).each do |attrs|
        sell_ctrt_no = attrs[:sell_ctrt_no].to_s.strip.upcase
        record = StdSellContract.find_by(sell_ctrt_no: sell_ctrt_no)
        if record.nil?
          errors << "매출계약을 찾을 수 없습니다: #{sell_ctrt_no}"
          next
        end

        update_attrs = contract_params_from_row(attrs)
        update_attrs.delete(:sell_ctrt_no)
        ...
        if record.save
          save_change_histories!(
            sell_contract: record,
            tracked_changes: tracked_changes,
            table_name: "std_sell_contracts"
          )
          result[:updated] += 1
        else
          errors.concat(record.errors.full_messages)
      ...
      Array(operations[:rowsToDelete]).each do |entry|
        sell_ctrt_no = extract_contract_no(entry)
        if sell_ctrt_no.blank?
          next
        end
        ...
        if record.update(use_yn_cd: "N")
          ...
          errors.concat(record.errors.full_messages.presence || [ "매출계약 비활성화에 실패했습니다: #{sell_ctrt_no}" ])
        end
      end
    ...
    if errors.any?
      ...
      render json: { success: true, message: "매출계약 데이터가 저장되었습니다.", data: result }
    end
  end
  ...
  def batch_save_settlements
    ...
    ActiveRecord::Base.transaction do
      ...
      Array(operations[:rowsToUpdate]).each do |attrs|
        ...
        if settlement.nil?
          errors << "정산정보를 찾을 수 없습니다: #{contract.sell_ctrt_no}/#{seq_no}"
          next
        end
        ...
        if settlement.save
          if tracked_changes.present?
            save_change_histories!(
              ...
              table_name: "std_sell_contract_settlements"
            )
          end
          result[:updated] += 1
        else
      ...
      Array(operations[:rowsToDelete]).each do |seq_no|
        ...
        if settlement.destroy
          ...
          errors.concat(settlement.errors.full_messages.presence || [ "정산정보 삭제에 실패했습니다: #{contract.sell_ctrt_no}/#{seq_no}" ])
        end
      end
    ...
    if errors.any?
      ...
      render json: { success: true, message: "매출계약 정산정보가 저장되었습니다.", data: result }
    end
  end
  ...
  private
    ...
    def find_contract
      sell_ctrt_no = params[:id].to_s.strip.upcase
      StdSellContract.find_by!(sell_ctrt_no: sell_ctrt_no)
    end
    ...
    def sell_contract_scope
      scope = StdSellContract.ordered
      ...
      if search_sell_ctrt_no.present?
        scope = scope.where("sell_ctrt_no LIKE ?", "%#{search_sell_ctrt_no}%")
      end
    ...
    def search_sell_ctrt_no
      value = search_params[:sell_ctrt_no].presence || search_params[:sellCtrtNo].presence
      value.to_s.strip.upcase.presence
    end
    ...
    def contract_permitted_fields
      [
        :corp_cd, :bzac_cd, :sell_ctrt_no, :sell_ctrt_nm, :bizman_no, :ctrt_sctn_cd, :ctrt_kind_cd,
    ...
    def settlement_permitted_fields
      [
        ...
        :main_bank_yn_cd, :exca_ofcr_cd, :exca_ofcr_nm, :use_yn_cd, :remk
      ]
    end
    ...
    def sell_contract_json(row)
      {
        id: row.sell_ctrt_no,
        corp_cd: row.corp_cd,
        bzac_cd: row.bzac_cd,
        bzac_nm: resolve_client_name(row.bzac_cd),
        sell_ctrt_no: row.sell_ctrt_no,
        sell_ctrt_nm: row.sell_ctrt_nm,
        ...
        ord_recp_poss_ymd: row.ord_recp_poss_ymd,
        ...
        ctrt_amt_chg_reason: row.ctrt_amt_chg_reason,
        main_rsbt_clause: row.main_rsbt_clause,
        re_ctrt_cond: row.re_ctrt_cond,
        ctrt_cnctr_cond: row.ctrt_cnctr_cond,
        ctrt_cnctr_dtl_reason: row.ctrt_cnctr_dtl_reason,
        sell_bnfit_amt: row.sell_bnfit_amt,
        sell_bnfit_rt: row.sell_bnfit_rt,
        contrbtn_bnfit_amt: row.contrbtn_bnfit_amt,
        contrbtn_bnfit_rt: row.contrbtn_bnfit_rt,
        remk: row.remk,
    ...
    def settlement_json(row)
      {
        id: "#{row.sell_contract_id}_#{row.seq_no}",
        seq_no: row.seq_no,
        ...
        main_bank_yn_cd: row.main_bank_yn_cd,
    ...
    def extract_contract_no(entry)
      if entry.respond_to?(:to_h)
        hash = entry.to_h
        value = hash["sell_ctrt_no"].presence || hash[:sell_ctrt_no].presence
      else
        value = entry
      end
    ...
    def next_settlement_seq(contract_id)
      StdSellContractSettlement.where(sell_contract_id: contract_id).maximum(:seq_no).to_i + 1
    end

    def save_change_histories!(sell_contract:, tracked_changes:, table_name:)
      if tracked_changes.blank?
        return
      end

      seq = StdSellContractChangeHistory.next_hist_seq_for(sell_contract.id)
      now = Time.current
      actor = current_actor

      tracked_changes.each do |column_name, values|
        before_value, after_value = values
        StdSellContractChangeHistory.create!(
