class Std::PurchaseContract::PageComponent < Std::BasePageComponent
  ...
  private
    ...
    def collection_path(**) = helpers.std_purchase_contracts_path(**)
    def member_path(id, **) = helpers.std_purchase_contract_path(id, **)

    def master_batch_save_url
      helpers.batch_save_std_purchase_contracts_path
    end

    def settlement_list_url_template
      "/std/purchase_contracts/:id/settlements.json"
    end

    def settlement_batch_url_template
      "/std/purchase_contracts/:id/batch_save_settlements"
    end

    def change_history_list_url_template
      "/std/purchase_contracts/:id/change_histories.json"
    end

    def selected_contract_label
      if selected_contract.present?
        "선택 매입계약: #{selected_contract}"
      else
        "매입계약을 먼저 선택하세요."
      end
    end
    ...
    def search_fields
      [
        ...
        { field: "bzac_nm", type: "input", label: "거래처명", placeholder: "거래처명 검색" },
        {
          ...
          type: "select",
          label: "계약구분",
          options: common_code_options("STD_PUR_CTRT_SCTN", include_all: true, all_label: "전체"),
          include_blank: false
        },
        { field: "pur_ctrt_no", type: "input", label: "매입계약번호", placeholder: "매입계약번호 검색" },
    ...
    def master_columns
      [
        ...
        { field: "pur_ctrt_no", headerName: "매입계약번호", minWidth: 140, editable: false },
        { field: "pur_ctrt_nm", headerName: "매입계약명", minWidth: 180, editable: false },
    ...
    def detail_form_model
      @detail_form_model ||= StdPurchaseContract.new
    end

    def detail_form_data
      {
        action: "submit->resource-form#submit submit->purchase-contract-grid#preventDetailSubmit"
      }
    end

    def basic_form_fields
      [
        { field: "pur_ctrt_no", type: "input", label: "매입계약번호", readonly: true, target: "detailField" },
        { field: "pur_ctrt_nm", type: "input", label: "매입계약명", required: true, maxlength: 200, target: "detailField" },
        ...
        { field: "bizman_no", type: "input", label: "사업자번호", required: true, maxlength: 20, inputmode: "numeric", target: "detailField" },
        {
          ...
          label: "계약구분",
          required: true,
          options: common_code_options("STD_PUR_CTRT_SCTN"),
          target: "detailField"
        },
        {
          field: "ctrt_kind_cd",
          ...
          label: "계약종류",
          required: true,
          options: common_code_options("STD_PUR_CTRT_KIND"),
          target: "detailField"
        },
        { field: "bef_ctrt_no", type: "input", label: "기존계약번호", maxlength: 30, target: "detailField" },
        { field: "cprtco_ofcr_cd", type: "input", label: "협력사담당자코드", maxlength: 50, target: "detailField" },
        ...
        { field: "ctrt_ofcr_cd", type: "input", label: "계약담당자코드", maxlength: 50, target: "detailField" },
        { field: "ctrt_ofcr_nm", type: "input", label: "계약담당자명", maxlength: 100, target: "detailField" },
    ...
    def additional_form_fields
      [
        { field: "ctrt_chg_reason_cd", type: "input", label: "계약변경사유", maxlength: 255, target: "detailField" },
        { field: "op_area_cd", type: "input", label: "운영권역", maxlength: 100, target: "detailField" },
        { field: "re_ctrt_cond_cd", type: "textarea", label: "재계약조건", rows: 2, target: "detailField" },
        { field: "ctrt_cnctr_cond_cd", type: "textarea", label: "계약해지조건", rows: 2, target: "detailField" },
        { field: "ctrt_cnctr_dtl_reason_cd", type: "textarea", label: "계약해지상세사유", rows: 2, target: "detailField" },
        {
          field: "pay_cond_cd",
          type: "select",
          label: "지급조건",
          options: common_code_options("STD_PAY_COND"),
          target: "detailField"
        },
        {
          field: "bzac_sctn_cd",
        ...
        { field: "work_step_no1_cd", type: "input", label: "작업단계번호1", maxlength: 30, target: "detailField" },
        { field: "work_step_no2_cd", type: "input", label: "작업단계번호2", maxlength: 30, target: "detailField" },
