...
export default class extends Controller {
  ...
  addRow() {
    ...
    this.gridApi.startEditingCell({ rowIndex: 0, colKey: "role_cd" })
  }
  ...
  deleteRows() {
    ...
    selectedNodes.forEach((node) => {
      ...
      if (row.role_cd) this.deletedRoleCodes.push(row.role_cd)
  ...
  buildOperations() {
    ...
    const rowsToDelete = [
      ...this.deletedRoleCodes,
      ...rows.filter((row) => row.__is_deleted && row.role_cd).map((row) => row.role_cd)
    ]
  ...
  pickFields(row) {
    return {
      role_cd: (row.role_cd || "").trim(),
      role_nm: (row.role_nm || "").trim(),
  ...
  rowChanged(row) {
    const original = this.originalMap.get(row.role_cd)
    if (!original) return true

    return (
      (row.role_nm || "") !== (original.role_nm || "") ||
  ...
  resetTracking() {
    ...
    this.collectRows().forEach((row) => {
      if (row.role_cd) this.originalMap.set(row.role_cd, { ...row })
      delete row.__is_new
      delete row.__is_updated
  ...
  handleCellValueChanged = (event) => {
    if (this.preventInvalidRoleCodeEdit(event)) return
    this.markRowUpdated(event)
  }

  preventInvalidRoleCodeEdit(event) {
    if (event?.colDef?.field !== "role_cd") return false
    if (!event?.node?.data) return false

    const row = event.node.data
    if (row.__is_new) return false

    row.role_cd = event.oldValue || ""
    this.gridApi.refreshCells({
      rowNodes: [event.node],
      columns: ["role_cd"],
      force: true
    })
    alert("기존 역할코드는 수정할 수 없습니다.")
    return true
  }
  ...
  isBlankRow(row) {
    return (row.role_cd || "").trim() === "" && (row.role_nm || "").trim() === ""
  }
